...
function unlockAudio(){try{if(!audioContext){audioContext=new(window.AudioContext||window.webkitAudioContext)();master=audioContext.createGain();master.gain.value=muted?0:.16;master.connect(audioContext.destination);}audioContext.resume().catch(()=>{});}catch{/* Audio is optional on unsupported browsers. */}}
function tone(note,duration=.1){if(!audioContext||audioContext.state!=='running')return;const o=audioContext.createOscillator(),g=audioContext.createGain(),now=audioContext.currentTime;o.type='triangle';o.frequency.value=440*2**((note-69)/12);g.gain.setValueAtTime(0,now);g.gain.linearRampToValueAtTime(.65,now+.012);g.gain.exponentialRampToValueAtTime(.001,now+duration);o.connect(g);g.connect(master);voices.push(o);o.onended=()=>{voices=voices.filter(v=>v!==o);o.disconnect();g.disconnect();};o.start();o.stop(now+duration+.02);}
...
function start(mode='A'){if(portraitScreen.matches)return;timePresses=0;unlockAudio();silence();Object.assign(state,{phase:'playing',score:0,misses:0,lane:3,fruits:[],time:0,spawn:.7,flash:0,mode});musicIndex=0;musicWait=0;refresh();}
...
function update(dt){if(state.phase==='paused'||document.hidden||portraitScreen.matches)return;state.time+=dt;state.flash=Math.max(0,state.flash-dt);if(state.phase==='birthday'){musicWait-=dt;if(musicWait<=0){const [note,beats]=melody[musicIndex];tone(note,beats*.38);musicWait=beats*.42;musicIndex++;if(musicIndex===melody.length){musicIndex=0;musicWait+=2;}}return;}if(state.phase!=='playing')return;
...
soundButton.addEventListener('click',()=>{unlockAudio();muted=!muted;if(master)master.gain.value=muted?0:.16;soundButton.textContent=muted?'Звук: выкл':'Звук: вкл';soundButton.setAttribute('aria-pressed',String(muted));});
